...
const Charts = {
  ...
  seasonal(el, {years = [], byYear = {}, yUnit = "", xLabels, endpoint = false} = {}) {
    ...
    return _mount(el, Object.assign(_base(), {
      ...
      series: (() => {
        ...
        if (endpoint) {                                  // 累计图：当年线端点带年份标签（参考定期跟踪看板）
          const cv = map[cur] || [];
          let li = -1;
          for (let i = cv.length - 1; i >= 0; i--) if (cv[i] !== null && cv[i] !== undefined) { li = i; break; }
          if (li > 0) series.push({name: "_endpoint", type: "scatter", silent: true,
            symbolSize: 7, itemStyle: {color: C_BLUE, borderColor: "#fff", borderWidth: 1},
            label: {show: true, position: "right", formatter: cur, fontSize: 10, color: C_BLUE},
            data: [[labels[Math.min(li, labels.length - 1)], cv[li]]]});
        }
        return series;
      })(),
      legend: {top: 0, icon: "roundRect", itemWidth: 14, itemHeight: 4, textStyle: {color: C_TEXT},
        data: ys.map(y => ({name: y}))},
      grid: {left: 8, right: 34, top: 30, bottom: 2, containLabel: true},
    }));
  },
