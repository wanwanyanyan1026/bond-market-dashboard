/* ============================================================
   ...
   ⑥ 政府债发行加权期限（MCP primary-tenor）：月度按期限档中点加权
     WAL 三线（国债/地方债/合计），Charts.line 全幅
...
PANELS["fiscal"] = {
  ...
  render(root) {
    ...
    /* —— ⑥ 政府债发行加权期限（MCP primary-tenor）：月度 WAL 三线 —— */
    const tenor = (F.primaryTenor && F.primaryTenor.monthly) || {};
    const tenorBox = h("div", {id: "fiscal-tenor-chart", class: "chart"});
    const tenorHas = (tenor.G && tenor.G.length) || (tenor.L && tenor.L.length) || (tenor.gov && tenor.gov.length);
    const tenorCard = h("section", {class: "card", id: "fiscal-tenor"}, [
      h("h3", {class: "card-title"}, ["政府债发行加权期限"]),
      App.badge("遇见投资MCP · primary-tenor", fetchedAt),
      h("p", {class: "card-sub"}, ["按期限档中点加权（1年以内0.5/1-3年2/3-7年5/7-10年8.5/10年以上15）· 月度 · 单位年"]),
      tenorHas ? tenorBox : h("div", {class: "empty"}, ["加权期限数据待接入"]),
    ]);
    function drawTenor() {
      if (!tenorHas) return;
      Charts.line(tenorBox, {series: [
        {name: "国债", dates: (tenor.G || []).map((p) => p.date), values: (tenor.G || []).map((p) => p.value)},
        {name: "地方债", dates: (tenor.L || []).map((p) => p.date), values: (tenor.L || []).map((p) => p.value)},
        {name: "合计", dates: (tenor.gov || []).map((p) => p.date), values: (tenor.gov || []).map((p) => p.value)},
      ], yUnit: "年"});
    }
